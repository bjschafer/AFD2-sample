...
while IFS= read -r -d $'\0' file; do
	[[ "$file" != *.vault && "$file" != *.vault.yml && "$file" != *.vault.yaml ]] && continue
	...
	head -1 "$file" | grep --quiet '^\$ANSIBLE_VAULT;' || {
        ...
        # ansible-vault view respects PAGER, so we set to cat so we can pipe it
        # grep checks if there are lines NOT matching (anything that's not a -): anything
        # it also excludes empty lines
        # if there are ANY lines that don't match this regex, it'll fail the hook.
        PAGER=cat ansible-vault view "$file" | grep -Eqv '^([^-]+:.*|\s*)$' && {
            echo "You have one or more vault tokens with a '-' in them, in $file"
            echo "'-' is not a valid character; you probably want '_'"
            EXIT_STATUS=1
        }
